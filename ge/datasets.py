...
class _Datasets(abc.ABC):
    def __init__(self, data_params: _DataParams, filters: Union[List[_Operator], List[type]],
                 db_connector: _DataBaseConnector = None, cs_connector: _CloudStorageConnector = None,
                 verbose: bool = True):
        self.data_params = data_params
        self.op_pipeline = OperatorPipeline(filters=filters, params=data_params)
...
class Acct2AssetDatasets(_Datasets):
    def __init__(self, data_params: Acct2AssetDataParams, filters: Union[List[type], List[_Operator]],
                 db_connector: _DataBaseConnector = None, cs_connector: _CloudStorageConnector = None,
                 verbose: bool = True):
        super().__init__(data_params=data_params, filters=filters,
